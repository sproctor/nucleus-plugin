...
internal const val POTASSIUM_TASK_GROUP = "potassium"
...
private fun JvmApplicationContext.configurePackagingTasks(commonTasks: CommonJvmDesktopTasks) {
    ...
    val allEbFormats =
        app.nativeDistributions.currentOsTargetFormats
    ...
    val createDistributable =
        tasks.register<AbstractJPackageTask>(
            ...
            args = listOf(TargetFormat.JpackageImage),
        ) {
...
private fun JvmApplicationContext.configureElectronBuilderPackageTask(
    ...
    packageTask.customNodePath.set(PotassiumProperties.electronBuilderNodePath(project.providers))
    packageTask.publishMode.set(PotassiumProperties.electronBuilderPublishMode(project.providers))
    ...
    when (currentOS) {
        ...
        OS.MacOS -> {
            app.nativeDistributions.macOS.also { mac ->
                ...
                // The jpackage task always builds a JpackageImage, so targetFormat.isStoreFormat
